...
**Companion document:** `docs/kyrgyzstan-state-standard-research.md` checks all of this against Kyrgyzstan's own state math standard, its 12-year-system transition, its university-entrance exam (ОРТ), and its current (2025) PISA results — read it alongside this one; several conclusions below (§4's grade-banding, §2's emphasis) are revised by what it found.
...
>
> **This is no longer just a theoretical risk — Kyrgyzstan's own PISA 2025 data makes it concrete** (`docs/kyrgyzstan-state-standard-research.md`): 49% of 15-year-olds scored below Level 2 (basic functional numeracy) in mathematics. With roughly half the relevant population below the functional-literacy floor, a curriculum that over-indexes its early production effort on rich open-ended/TWM/founder-adjacent tasks — which mainly benefit students who already have a functional base to reason from — would optimize for the wrong end of the actual distribution. The correct read of this section is: build the scaffolded-fluency and functional-literacy layers first, at scale, across the full distribution; treat the founder/enrichment layer described here as the ceiling for students who are ready for it, not the platform's primary pitch.
...
6. **No quantitative-comparison activity type.** `docs/kyrgyzstan-state-standard-research.md` confirms Kyrgyzstan's own university-entrance exam (ОРТ, 60 math questions/90 minutes) uses a distinct item format — two quantities in side-by-side boxed columns, judge their relationship without necessarily computing an exact value — that has no equivalent in the current `ACTIVITY_TYPES` list. It's a good vehicle for exactly the structure-spotting-without-brute-force habit this document already wants (§2), and it's directly exam-relevant, which most of the other additions aren't. **Recommend adding a `quant_compare` activity type**, concentrated in grades 9–11 where ОРТ prep is live.
...
> **This grade-banding is now known to be provisional.** `docs/kyrgyzstan-state-standard-research.md` confirms Kyrgyzstan is mid-transition to a **12-year system** (new standard live from 2025/2026, rolling out grade-by-grade, starting with grades 1, 2, 5, and 8), so the old 4+5+2 structure below should be treated as a placeholder for the *strand logic*, not a final grade map. Re-derive the grade numbers once the 12-year structure is confirmed from the primary source, and prioritize authoring for grades 1, 2, 5, and 8 first regardless — that's where new official material is landing and where a MathKG resource has the least entrenched competition. Also rename the strand column headers below to the state standard's own content-line names once the full list is confirmed (so far: "числа и вычисления," "алгебраические отношения и выражения," "геометрические фигуры и их свойства," "геометрические величины и их измерения") — matching official vocabulary is what makes a `Topic` legible to a Kyrgyz teacher as covering a named содержательная линия, not a parallel taxonomy.
...
1. **~~Blocking~~ Partially addressed — see `docs/kyrgyzstan-state-standard-research.md`.** That document confirms the legal basis (subject standards approved by Resolution №393/2022 for grades 5–9, a 2021–2022 revision for grades 1–4, and a new overarching State Educational Standard under Resolution №131/2025), the official content-line names, the ОРТ exam's actual structure, and current PISA 2025 data (Kyrgyzstan: 364 in math, 49% of 15-year-olds below Level 2). It also surfaces a fact that changes §4 directly: Kyrgyzstan is mid-transition from an 11-year to a **12-year school system**, rolling out grade-by-grade with grades 1, 2, 5, and 8 first — so §4's grade-1–11 banding needs to be re-derived once the new structure is confirmed, and content-line labels should be renamed to match the state standard's own terms rather than the ad hoc English-style labels below. It also could not retrieve the full grade-by-grade content breakdown (network access to the primary-source PDFs was blocked in the research session) — that remains open and is the one piece still genuinely outstanding before scope-and-sequence authoring should be finalized.
